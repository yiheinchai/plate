  ...
  return (
    <section className="flex flex-col gap-2.5">
      <div>
        ...
        <p className="text-[11px] text-text-muted/60 mt-0.5">Speech-to-text engine configuration</p>
      </div>
      <div className="bg-bg-card/40 rounded-xl border border-border-subtle/40 p-4 flex flex-col gap-4">
        {/* Engine toggle */}
        <div className="flex flex-col gap-1.5">
          <span className="text-[13px] text-text-secondary">Engine</span>
          <div className="relative flex bg-bg-primary rounded-lg p-0.5 border border-border-subtle/40">
            <div
              ...
              style={{
                left: settings.transcription_engine === "whisper_local" ? "2px" : "50%",
                width: "calc(50% - 2px)",
              }}
            />
            <button
        ...
        {settings.transcription_engine === "whisper_local" && (
          <label className="flex flex-col gap-1.5" style={{ animation: "fade-in 0.15s ease-out" }}>
            <span className="text-[13px] text-text-secondary">Whisper Model</span>
            <select
        ...
        {settings.transcription_engine === "whisper_api" && (
          <label className="flex flex-col gap-1.5" style={{ animation: "fade-in 0.15s ease-out" }}>
            <span className="text-[13px] text-text-secondary">OpenAI API Key</span>
